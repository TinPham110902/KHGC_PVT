            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="">
                        ...
                        <div class="row mb-3">
                            <label for="address" class="col-md-4 col-form-label text-md-end">{{ __('Status') }}</label>

                            <div class="col-md-6">
                                <select class="form-control" name="status" id="exampleFormControlSelect1">
                                    <option {{ $users->status->name == 'WAITING' ? 'selected' : '' }} value="0">WAITING
                                    </option>

                                    <option {{ $users->status->name == 'ALLOWED' ? 'selected' : '' }} value="1">ALLOWED
                                    </option>

                                    <option {{ $users->status->name == 'DENIED' ? 'selected' : '' }} value="2">DENIED
                                    </option>

                                    <option {{ $users->status->name == 'BLOCKED' ? 'selected' : '' }} value="3">BLOCKED
                                    </option>
                                </select>
                            </div>
                        </div>
